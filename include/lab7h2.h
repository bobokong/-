...
/*初始化队列：创建一空队列*/
void InitQueue(Queue) struct QUEUE *Queue;
{
...
/*清空队列*/
void Clear(Queue) struct QUEUE *Queue;
{
    Queue->Front = MAX_LEN - 1;
    Queue->Rear = MAX_LEN - 1;
}
...
/*入队操作：在队尾增加一个新元素x */
void Add(Queue, x) struct QUEUE *Queue;
int x;
{
    if (IsFull(Queue)) {
        printf("Queue Full!\n");
...
void Del(Queue, y) struct QUEUE *Queue;
int *y;
{
    if (IsEmpty(Queue)) {
        printf("Queue Empty!\n");
...
/*打印循环队列：基于数组结构*/
void PrintNode(Queue) struct QUEUE *Queue;
{
    int i;
...
/*打印循环队列：基于队列结构*/
void PrintNode_1(Queue) struct QUEUE *Queue;
{
    int i;
